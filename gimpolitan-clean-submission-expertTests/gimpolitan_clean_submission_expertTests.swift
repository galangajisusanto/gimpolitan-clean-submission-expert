//
//  gimpolitan_clean_submission_expertTests.swift
//  gimpolitan-clean-submission-expertTests
//
...
@testable import gimpolitan_clean_submission_expert
class RemoteDataSourceTest: XCTestCase {
   ...
   """
    ...
    static var fakeResponse: GameDomainResponse = GameDomainResponse(
        idGame: 1,
        name: "test",
...
extension RemoteDataSourceTest {
    ...
    class GetGamesRemoteMock: DomainRemoteDataSource {
        ...
        func execute(request: String?) -> Observable<[GameDomainResponse]> {
            return Observable<[GameDomainResponse]>.create {observer in
                
                if !self._endpoint.isEmpty {
                    if !self._key.isEmpty {
                        do {
                            let value = try JSONDecoder().decode(GameListResponse.self, from: Data(responseJson.utf8))
